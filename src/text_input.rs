//! Single-line text input for GPUI 0.2.
//!
//! GPUI has no built-in `TextInput`. This follows `gpui/examples/input.rs`:
//! `EntityInputHandler` for IME/typed characters, a custom `Element` for the
//! caret and selection, and key bindings for arrows, select-all, copy, cut, paste.
...
impl TextInput {
    ...
    fn content_index_to_display(&self, content_idx: usize) -> usize {
        if !self.masked || self.content.is_empty() {
            return content_idx.min(self.content.len());
        }
        let idx = content_idx.min(self.content.len());
        let char_idx = self.content[..idx].chars().count();
        char_idx * MASK_CHAR.len_utf8()
    }

    fn display_index_to_content(&self, display_idx: usize) -> usize {
        if !self.masked {
            return display_idx.min(self.content.len());
        }
        let char_idx = display_idx / MASK_CHAR.len_utf8();
        self.content
            .chars()
            .take(char_idx)
            .map(|c| c.len_utf8())
    ...
    fn index_for_mouse_position(&self, position: Point<Pixels>) -> usize {
        ...
        self.display_index_to_content(display_idx)
    }
...
impl EntityInputHandler for TextInput {
    ...
    fn bounds_for_range(
        ...
        let range = self.range_from_utf16(&range_utf16);
        let start = self.content_index_to_display(range.start);
        let end = self.content_index_to_display(range.end);
    ...
    fn character_index_for_point(
        ...
        Some(self.offset_to_utf16(self.display_index_to_content(utf8_index)))
    }
}
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if content.is_empty() {
            (input.placeholder.clone(), rgb(0x8f8fa3).into())
        } else {
        ...
        let runs = if let Some(marked_range) = input.marked_range.as_ref() {
            if input.masked || content.is_empty() {
                vec![run]
            } else {
                vec![
                    TextRun {
        ...
        let cursor_pos = line.x_for_index(input.content_index_to_display(cursor));
